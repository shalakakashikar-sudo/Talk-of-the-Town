...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      ...
      try {
        ...
        console.error("Failed to load city background");
      } finally {
  ...
  const selectMode = async (mode: GameMode) => {
    ...
    try {
      ...
      setError(err.message || "Connection failure. Check your Neural Link status.");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || "Linguistic relay interrupted.");
    } finally {
      setIsTyping(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-gray-950">
        ...
        <div className="max-w-4xl w-full glass rounded-[3rem] shadow-2xl p-8 md:p-14 border border-white/10 text-center relative z-10 animate-fade-in">
            ...
            {Object.values(GameMode).map((mode) => (
              ...
              >
                <div className="relative z-10">
                  ...
                  <p className="text-sm text-gray-400 group-hover:text-gray-200 transition-colors font-medium leading-relaxed">
          ...
          {error && (
            <div className="mt-12 p-8 bg-rose-950/40 border-2 border-rose-500/30 rounded-3xl text-rose-200 text-sm animate-fade-in shadow-[0_0_40px_rgba(244,63,94,0.1)]">
              <div className="flex items-center justify-center space-x-3 mb-4">
                ...
                <span className="font-black uppercase tracking-[0.2em] text-xs">Neural Link Failure</span>
              </div>
              <p className="mb-4 leading-relaxed font-medium">{error}</p>
              <div className="pt-4 border-t border-rose-500/20">
                <p className="text-[10px] opacity-70 uppercase font-black tracking-widest leading-loose">
                  Troubleshooting Tip:<br/>
                  1. Visit Vercel Dashboard -> Settings -> Environment Variables.<br/>
                  2. Verify 'API_KEY' is set to a value starting with 'AIza'.<br/>
                  3. **IMPORTANT**: Go to the 'Deployments' tab and click 'Redeploy'.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="max-w-4xl mx-auto space-y-12 pb-12">
            ...
            >
              <div 
                className={`max-w-[90%] md:max-w-[85%] rounded-[2.5rem] p-6 md:p-10 shadow-2xl transition-all ${
                  msg.role === 'user' 
                    ? 'bg-indigo-600 text-white rounded-tr-none border-b-8 border-indigo-800 shadow-indigo-500/10' 
                    : 'bg-gray-800/50 glass text-gray-100 border border-white/5 rounded-tl-none border-b-8 border-gray-950 shadow-black/40'
                }`}
              >
          ...
          {isTyping && (
            <div className="flex items-center space-x-3 text-indigo-400 bg-indigo-950/20 px-6 py-3 rounded-full w-fit border border-indigo-500/20 shadow-xl backdrop-blur-sm animate-pulse">
              <span className="text-[10px] font-black uppercase tracking-[0.3em]">Processing Neural Feed...</span>
            </div>
          )}

          {error && (
            <div className="bg-rose-950/20 border border-rose-500/20 p-6 rounded-3xl text-rose-300 text-sm flex items-center space-x-4 animate-fade-in">
        ...
        <div className="max-w-4xl mx-auto">
          <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-4">
            <input
              ...
              placeholder="Respond to the simulation..."
