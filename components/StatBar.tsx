...
const StatBar: React.FC<StatBarProps> = ({ stats, onExit }) => {
  return (
    <div className="bg-gray-950/80 border-b border-white/5 p-4 md:px-8 md:py-6 sticky top-0 z-50 shadow-2xl backdrop-blur-2xl">
      <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-6">
        <div className="flex items-center space-x-8">
          <button 
            onClick={onExit}
            className="flex items-center space-x-3 text-gray-400 hover:text-white transition-all bg-white/5 hover:bg-white/10 px-5 py-2.5 rounded-2xl group border border-white/5 shadow-sm"
          ...
          <div className="hidden xl:flex flex-col">
            <span className="text-[10px] uppercase font-black text-gray-600 tracking-widest mb-1">Creator</span>
        ...
        <div className="flex items-center space-x-10">
          <div className="flex flex-col items-end">
             <div className="flex items-center space-x-3 mb-2">
                ...
                <span className="text-sm font-mono font-black text-white">{stats.confidence}%</span>
             </div>
             <div className="w-40 h-2 bg-white/5 rounded-full overflow-hidden border border-white/5 shadow-inner">
                <div 
                  className={`h-full transition-all duration-1000 ${stats.confidence > 75 ? 'bg-emerald-500' : stats.confidence > 40 ? 'bg-amber-500' : 'bg-rose-500 shadow-[0_0_15px_rgba(244,63,94,0.5)]'}`}
          ...
          <div className="flex flex-col items-end group">
            <span className="text-[10px] uppercase font-black text-gray-600 tracking-widest mb-1 group-hover:text-emerald-500 transition-colors">Current Zone</span>
